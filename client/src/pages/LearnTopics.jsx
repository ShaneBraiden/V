/** @fileoverview Topic grid for a single technology — /learn/:techId */
...
export default function LearnTopics() {
    ...
    const totalXP = tech.topics.reduce((s, t) => s + t.xp, 0);
    const earnedXP = tech.topics.filter(t => completed[`${techId}-${t.id}`]).reduce((s, t) => s + t.xp, 0);
    const completedCount = tech.topics.filter(t => completed[`${techId}-${t.id}`]).length;
    const overallPct = Math.round((completedCount / tech.topics.length) * 100);
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {tech.topics.map((topic, idx) => {
                    const done = !!completed[`${techId}-${topic.id}`];
                    const locked = idx > 0 && !completed[`${techId}-${tech.topics[idx - 1].id}`] && !done;
